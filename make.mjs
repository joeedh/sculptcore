...
// Default extra sbrush kernel dirs (SCULPTCORE_EXTRA_KERNEL_DIRS); a string
// (path.delimiter-separated) or array. The --kernels-extra CLI flag overrides.
const EXTRA_KERNEL_DIRS = getopt('EXTRA_KERNEL_DIRS', undefined)
...
/* --kernels-extra dirs -> the SCULPTCORE_EXTRA_KERNEL_DIRS cmake value:
 * absolute forward-slash paths joined with ';' (relative paths resolve against
 * the engine root). undefined = flag absent and no EXTRA_KERNEL_DIRS local
 * option — leave any configured value alone; '' = explicitly none (clears a
 * previously configured value). */
function resolveKernelsExtra(argVal) {
  let dirs = argVal !== undefined ? argVal : EXTRA_KERNEL_DIRS
  if (dirs === undefined) {
    return undefined
  }
  ...
  const out = []
  for (const d of dirs) {
    for (const part of String(d).split(Path.delimiter)) {
      if (part.trim()) {
        out.push(Path.resolve(part.trim()).replace(/\\/g, '/'))
      }
    }
  }
  return out.join(';')
...
// Read one variable from a build dir's CMakeCache.txt (undefined if absent).
function readCmakeCacheVar(dir, name) {
  const cache = Path.join(dir, 'CMakeCache.txt')
  if (!fs.existsSync(cache)) {
    return undefined
  }
  for (const line of fs.readFileSync(cache, 'utf-8').split(/\r?\n/)) {
...
// like the node addon. No host-side sbrush codegen pass: built-in kernels use
// the checked-in .gen.h headers (regenerated by `codegen`/`build native`);
// extra kernels (SCULPTCORE_EXTRA_KERNEL_DIRS) are generated in-build.
async function buildPythonCapi(kernelsExtra) {
  const dir = buildDir('python')
  const env = envPrefix('python')

  if (!fs.existsSync(Path.join(dir, 'CMakeCache.txt'))) {
    await configureTarget('python', {kernelsExtra})
  } else {
    await ensureKernelsExtraConfigured('python', kernelsExtra)
  }
...
async function bundleAddon(dest, {build, pdb, publishDepsTo, kernelsExtra}) {
  if (build) {
    await buildPythonCapi(kernelsExtra)
  }
...
async function configureTarget(target, {backends, runtime, runtimeVersion, kernelsExtra}) {
  target = normalizeTarget(target)
  ensureDir('build')
  if (target === 'node') {
  ...
  if (target === 'native' || target === 'python') {
    ...
    if (kernelsExtra !== undefined) {
      NATIVE_CMAKE_ARGS += ` -DSCULPTCORE_EXTRA_KERNEL_DIRS="${kernelsExtra}"`
    }

    run(`cd ${dir} && ${env} cmake ../.. ${NATIVE_CMAKE_ARGS}`)
  } else {
    if (kernelsExtra) {
      console.warn('configure: --kernels-extra is native-only; ignored for wasm')
    }
    run(
...
/* Reconfigure `target` when its cached SCULPTCORE_EXTRA_KERNEL_DIRS doesn't
 * match the requested value, so --kernels-extra changes take effect without a
 * manual configure. undefined (flag absent) keeps whatever is configured. */
async function ensureKernelsExtraConfigured(target, kernelsExtra) {
  if (kernelsExtra === undefined) {
    return
  }
  const dir = buildDir(target)
  if (!fs.existsSync(Path.join(dir, 'CMakeCache.txt'))) {
    return // the on-demand configure passes it
  }
  const cached = readCmakeCacheVar(dir, 'SCULPTCORE_EXTRA_KERNEL_DIRS') ?? ''
  if (cached !== kernelsExtra) {
    console.log(`build: ${dir} extra kernel dirs "${cached}" -> "${kernelsExtra}" — reconfiguring`)
    await configureTarget(target, {kernelsExtra})
  }
}
...
// Shared --kernels-extra option (configure / build / bundle).
const kernelsExtraOption = (y) =>
  y.option('kernels-extra', {
    type    : 'string',
    array   : true,
    describe:
...
yargs(hideBin(process.argv))
  ...
  .command(
    ...
    (y) =>
      kernelsExtraOption(nodeRuntimeOptions(configureTargetPositional(y))).option('backends', {
        type    : 'string',
        describe: `comma-separated sbrush backends to enable (subset of: ${SBRUSH_BACKENDS.join(',')}); cpp is always on`,
      }),
    async ({target, backends, runtime, runtimeVersion, kernelsExtra}) => {
      setupPNPM()
      const kx = resolveKernelsExtra(kernelsExtra)
      const targets = target ? [normalizeTarget(target)] : ['wasm', 'native', 'node']
      for (const t of targets) {
        await configureTarget(t, {backends, runtime, runtimeVersion, kernelsExtra: kx})
      }
    }
  )
  .command(
  ...
  .command(
    ...
    (y) =>
      kernelsExtraOption(nodeRuntimeOptions(buildTargetPositional(y))).option('smoke', {
        type    : 'boolean',
        default : false,
        describe: 'node target only: after building, load the .node under NW.js and run the napi smoke',
      }),
    async ({target, runtime, runtimeVersion, smoke, kernelsExtra}) => {
      target = normalizeTarget(target)
      const kx = resolveKernelsExtra(kernelsExtra)
      if (target === 'node') {
        if (kx) {
          console.warn('build: --kernels-extra is not wired for the node target; ignored')
        }
        await sbrushCodegen()
      ...
      if (target === 'python') {
        await buildPythonCapi(kx)
        return
      }
      if (target === 'native') {
        await ensureKernelsExtraConfigured('native', kx)
      } else if (kx) {
        console.warn('build: --kernels-extra is native-only; ignored for wasm')
      }
  ...
  .command(
    ...
    async ({dest, build, pdb, publishDepsTo, kernelsExtra}) => {
      await bundleAddon(dest, {build, pdb, publishDepsTo, kernelsExtra: resolveKernelsExtra(kernelsExtra)})
    }
  )

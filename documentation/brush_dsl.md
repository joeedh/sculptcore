...
| `@planeFrame` | the kernel projects onto the plane through `surfacePos` along `surfaceNo`, so the executor may resolve that frame per dab from the stroke's plane-frame policy (`brush/plane_frame.h`: area-averaged normal and/or centre gathered around the cursor, a fixed view/axis normal, the stroke's first frame held, the PLANE type's rolling-average stabilisation — Blender's `calc_brush_plane`). Emits `def.usesPlaneFrame`. A kernel without it always sees the host's hit frame; the CPU executors are the only consumers, the WGSL path marshals the host frame directly |

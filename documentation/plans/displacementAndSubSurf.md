...
2. **Cross-field combing** (F3): signed frames need branch cuts; v1 places
   ...
   nudged cube cage — `test_multires`'s `gateFrameStability`). **Resolved for
   multires** by not asking the cross field at all: `Multires::parametricFrames()`
   derives the frame from the grid lattice, and is now what every multires encode
   and decode uses. F3 itself is unchanged and still serves VDM and brush
   consumers, so this risk stays open for *those* — and `captureDetailToVdm` now
   straddles the two frame spaces; see `CLAUDE.md` § *Subdivision and multires*.

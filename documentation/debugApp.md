...
| `set_brush`     | `radius=F strength=F spacing=F invert=0/1`<br>`pinch=F planeoff=F plane_height=F plane_depth=F rotate_angle=F` | tweaks the active `brush::Brush` props and re-syncs them through `props::StructProp`; `spacing` is the per-stroke fraction of the brush *diameter* between successive dabs (default 0.25); the second row are the kernel `@static` uniforms (crease/blob's signed `pinch`, the plane brush's reach above and below its offset plane, rotate's cumulative angle in radians) |
...
| `dyntopo`       | `enabled=0/1 detail=F [min=F] [grade=F] [flip=0/1]`<br>`[smooth=0/1] [max_splits=N] [graded=0/1] [pinch=0/1]`<br>`[mode=both\|subdivide\|collapse]` | configures dynamic-topology remesh applied as a pre-pass to subsequent strokes (`source/dyntopo/`). `pinch=1` cuts thin tubes at separating triangles and culls the small pieces left behind (`DynTopoParams::pinch_thin`) |
| `bench_dyntopo` | `detail=F radius=F center=x,y,z [grade=F flip=0/1`<br>`smooth=0/1 max_splits=N spatial=0/1 rebuild=0/1`<br>`graded=0/1]` | A/B one dyntopo dab vs a full tree rebuild; prints splits/flips/rounds/leftover/maxValence/CV + ops/update/total ms. The M7 measurement tool; `graded=1` switches the region rule to the Blender port (`DynTopoRegion::GradedRecursive`) |

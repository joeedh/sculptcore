...
fn brush_falloff_dist(delta: vec3<f32>) -> f32 {
  ...
  if (brush_u.falloff_shape == 1u) {
  ...
  } else if (brush_u.falloff_shape == 4u) {
    let sb_n = normalize(ctx_u.surfaceNo);
    var sb_tang = brush_u.falloff_dir - sb_n * dot(brush_u.falloff_dir, sb_n);
    var sb_tl = length(sb_tang);
    if (sb_tl < 1e-6) {
    ...
    sb_tang = sb_tang / sb_tl;
    let sb_lat = cross(sb_n, sb_tang);
    let sb_q0 = abs(dot(delta, sb_tang)) * sb_inv_r / brush_u.falloff_extent.x;
    let sb_q1 = abs(dot(delta, sb_lat)) * sb_inv_r / brush_u.falloff_extent.y;
    if (sb_q0 > 1.0 || sb_q1 > 1.0) { return 1.0; }
    let sb_core = 1.0 - ctx_u.falloff_roundness;
    let sb_e0 = select(0.0, sb_q0 - sb_core, sb_q0 > sb_core);
    let sb_e1 = select(0.0, sb_q1 - sb_core, sb_q1 > sb_core);
    if (sb_e0 == 0.0 && sb_e1 == 0.0) { return 0.0; }
    if (ctx_u.falloff_roundness <= 0.0) { return 1.0; }
    return min(sqrt(sb_e0 * sb_e0 + sb_e1 * sb_e1) / ctx_u.falloff_roundness, 1.0);
  }
  return length(delta) * sb_inv_r;
}

fn brush_inside_rounded_box(delta: vec3<f32>) -> bool {
  let sb_n = normalize(ctx_u.surfaceNo);
  var sb_tang = brush_u.falloff_dir - sb_n * dot(brush_u.falloff_dir, sb_n);
  var sb_tl = length(sb_tang);
  if (sb_tl < 1e-6) {
  ...
  sb_tang = sb_tang / sb_tl;
  let sb_lat = cross(sb_n, sb_tang);
  return abs(dot(delta, sb_tang)) <= brush_u.radius * brush_u.falloff_extent.x && abs(dot(delta, sb_lat)) <= brush_u.radius * brush_u.falloff_extent.y && abs(dot(delta, sb_n)) <= brush_u.radius * brush_u.falloff_extent.z;
}
...
fn brush_strength(p: vec3<f32>) -> f32 {
  ...
  if (brush_u.radius <= 0.0 || sb_dist > 1.0 || (brush_u.falloff_shape == 2u && length(sb_delta) > brush_u.radius) || (brush_u.falloff_shape == 4u && !brush_inside_rounded_box(sb_delta))) { return 0.0; }
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  }
  {
    ...
    for (var sb_ni0 = 0u; sb_ni0 < sb_nbr_meta0.y; sb_ni0 = sb_ni0 + 1u) {
      ...
      var disp: vec3<f32> = (co_prev[sb_nb_v0] - v_co);
...
`,
  "crease": `// AUTO-GENERATED by sbrushc (WGSL backend) — DO NOT EDIT.
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  var f: f32 = abs((s / brush_u.strength));
  var pull: vec3<f32> = (((ctx_u.surfacePos - v_co)) * ((f * brush_u.pinch)));
  pull -= (ctx_u.surfaceNo * dot(pull, ctx_u.surfaceNo));
  v_co += (pull + (ctx_u.surfaceNo * (((s * brush_u.radius) * 0.5))));
  ...
  }
  {
    ...
    for (var sb_ni0 = 0u; sb_ni0 < sb_nbr_meta0.y; sb_ni0 = sb_ni0 + 1u) {
      ...
      if (((wTan2 != 0.0) && (haveField > 0.5))) {
        var e: vec3<f32> = (co_prev[sb_nb_v0] - v_co);
        e = (e - (v_no * dot(e, v_no)));
        var el: f32 = length(e);
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  if (lid >= sb_node.vert_count) { return; }
  let sb_vidx = unique_verts[sb_node.vert_offset + lid];
  let sb_base: vec3<f32> = co_buf[sb_vidx] - sb_disp[sb_vidx];
  var v_co: vec3<f32> = sb_base;
  var v_no: vec3<f32> = no_buf[sb_vidx];
  var v_mask: f32 = mask_buf[sb_vidx];
  ...
  let sb_delta = select(v_co - sb_base, v_co - co_buf[sb_vidx], sb_first);
  co_buf[sb_vidx] = co_buf[sb_vidx] + sb_delta;
  sb_disp[sb_vidx] = sb_disp[sb_vidx] + sb_delta;
  no_buf[sb_vidx] = v_no;
  mask_buf[sb_vidx] = v_mask;
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  var d: vec3<f32> = (ctx_u.surfacePos - v_co);
  var across: vec3<f32> = cross(ctx_u.surfaceNo, ctx_u.strokeDir);
  var al: f32 = length(across);
  var t: vec3<f32> = d;
  if ((al > 0.0001)) {
    across = (across / al);
    t = ((across * dot(d, across)) + (ctx_u.surfaceNo * dot(d, ctx_u.surfaceNo)));
  }
  v_co += (t * s);
...
`,
  "planebrush": `// AUTO-GENERATED by sbrushc (WGSL backend) — DO NOT EDIT.
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  var P: vec3<f32> = (ctx_u.surfacePos + (ctx_u.surfaceNo * ((brush_u.planeoff * brush_u.radius))));
  var h: f32 = dot((v_co - P), ctx_u.surfaceNo);
  var reach: f32 = brush_u.planeDepth;
  if ((h >= 0.0)) {
    reach = brush_u.planeHeight;
  }
  if ((reach == 0.0)) {
    return;
  }
  var probe: vec3<f32> = ((v_co - (ctx_u.surfaceNo * dot((v_co - ctx_u.surfacePos), ctx_u.surfaceNo))) + (ctx_u.surfaceNo * ((h / reach))));
  var s: f32 = ((brush_strength(probe) * brush_masks(sb_vidx, v_mask)) * reach);
  if ((s == 0.0)) {
    return;
  }
  v_co -= (ctx_u.surfaceNo * ((h * s)));
...
`,
  "rotate": `// AUTO-GENERATED by sbrushc (WGSL backend) — DO NOT EDIT.
// Source: source/brush/kernels/rotate.sbrush
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  var a: f32 = (brush_u.rotateAngle * fall);
  var k: vec3<f32> = normalize(ctx_u.surfaceNo);
  var d: vec3<f32> = (v_co - ctx_u.surfacePos);
  var c: f32 = cos(a);
  var sn: f32 = sin(a);
  var r: vec3<f32> = (((d * c) + (cross(k, d) * sn)) + (k * ((dot(k, d) * ((1.0 - c))))));
  v_co += (r - d);
...
`,
  "texgrad": `// AUTO-GENERATED by sbrushc (WGSL backend) — DO NOT EDIT.
...
fn tex_rings_eval_d(p: sbdual3, n: sbdual3) -> sbdual {
  var d: sbdual = sbd_length(p);
  var rings: sbdual = sbd_add(sb_c(0.5), sbd_mul(sb_c(0.5), sbd_sin(sbd_mul(d, sb_c(40.0)))));
  var bands: sbdual = sbd_fract(sbd_mul(d, sb_c(6.0)));
  var steps: sbdual = sbd_mul(sbd_floor(sbd_mul(bands, sb_c(4.0))), sb_c(0.25));
  var tilt: sbdual = sbd_add(sb_c(0.5), sbd_mul(sb_c(0.5), sbd_cos(sbd_mul(sbd_dot(n, p), sb_c(8.0)))));
  return sbd_mul(sbd_mul(rings, steps), tilt);
}
    ...
    @builtin(workgroup_id) gid: vec3<u32>)
{
  ...
  var g: vec3<f32> = (tex_rings_eval_d(sb_seed3(v_co), sb_c3(ctx_u.surfaceNo))).d;
  v_co += (g * ((s * 0.0050000000000000001)));

...
static void quality(Mesh *m, float3 center, float r2, double &cv, double &minArea)
{
  ...
  int n = 0;
  for (int e : m->e) {
    if (m->e.c[e] == ELEM_NONE) continue;
    float3 a = m->v.co[m->e.vs[e][0]], b = m->v.co[m->e.vs[e][1]];
    if (((a + b) * 0.5f - center).lengthSqr() > r2) continue;
  ...
  for (int f : m->f) {
    ...
    if (((A + B + C) * (1.0f / 3.0f) - center).lengthSqr() > r2) continue;
    double area = 0.5 * double((B - A).cross(C - A).length());
    if (area < minArea) minArea = area;
  }
}

static int run(bool smooth, float3 center, float radius, double &cv, double &minArea,
               int &leftover, int &smooths)
{
  ...
  for (int e : m->e) {
    ...
    if ((a - b).length() > target * 1.001f) leftover++;
  }

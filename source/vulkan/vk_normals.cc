...
GpuNormalPass::~GpuNormalPass()
{
  ...
  for (Pipe *p : {&face_, &vert_, &scatter_}) {
    ...
    if (p->dsLayout)
      vkDestroyDescriptorSetLayout(d, p->dsLayout, nullptr);
  }
  if (pool_)
    vkDestroyDescriptorPool(d, pool_, nullptr);
  if (scatterPool_)
    vkDestroyDescriptorPool(d, scatterPool_, nullptr);
}
...
bool GpuNormalPass::init()
{
  ...
  if (!loadModule("normal_face.spv", mFace) || !loadModule("normal_vert.spv", mVert) ||
...
void GpuNormalPass::bindStorage(VkDescriptorSet set, uint32_t binding, VkBuffer buf)
{
...
bool GpuNormalPass::dispatch(const Pipe &p, uint32_t count)
{
  ...
  return ctx_->runOneShot([&](VkCommandBuffer cb) {
    ...
    vkCmdBindDescriptorSets(
        cb, VK_PIPELINE_BIND_POINT_COMPUTE, p.layout, 0, 1, &p.set, 0, nullptr);
    vkCmdPushConstants(
        cb, p.layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &count);
    vkCmdDispatch(cb, groups, 1, 1);
  });
}

bool GpuNormalPass::setTopology(const uint32_t *triVerts,
                                int triCount,
                                ...
                                int listCount)
{
  ...
  std::memcpy(triVerts_.mapped, triVerts, size_t(triCount) * 3 * sizeof(uint32_t));
  std::memcpy(vtriMeta_.mapped, vtriMeta, size_t(vertCount) * 2 * sizeof(uint32_t));
  if (listCount > 0) {
    std::memcpy(vtriList_.mapped, vtriList, size_t(listCount) * sizeof(uint32_t));
  }
  return true;
}

bool GpuNormalPass::prepareNormals(VkBuffer co,
                                   VkBuffer no,
                                   ...
                                   int vertWorkCount)
{
  ...
  if (!ensureBuf(workTris_,
                 VkDeviceSize(triWorkCount < 1 ? 1 : triWorkCount) * sizeof(uint32_t)) ||
      !ensureBuf(workVerts_,
                 VkDeviceSize(vertWorkCount < 1 ? 1 : vertWorkCount) * sizeof(uint32_t)))
  {
    return false;
  }
  if (triWorkCount > 0) {
    std::memcpy(workTris_.mapped, workTris, size_t(triWorkCount) * sizeof(uint32_t));
  }
  if (vertWorkCount > 0) {
    std::memcpy(workVerts_.mapped, workVerts, size_t(vertWorkCount) * sizeof(uint32_t));
  }
...
void GpuNormalPass::recordNormals(VkCommandBuffer cb)
{
  ...
  if (faceGroups > 0) {
    ...
    vkCmdBindDescriptorSets(
        cb, VK_PIPELINE_BIND_POINT_COMPUTE, face_.layout, 0, 1, &face_.set, 0, nullptr);
    vkCmdPushConstants(
        cb, face_.layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &faceCount);
  ...
  if (vertGroups > 0) {
    ...
    vkCmdBindDescriptorSets(
        cb, VK_PIPELINE_BIND_POINT_COMPUTE, vert_.layout, 0, 1, &vert_.set, 0, nullptr);
    vkCmdPushConstants(
        cb, vert_.layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &vertCount);
...
bool GpuNormalPass::computeNormals(VkBuffer co, VkBuffer no)
{
  ...
  for (int i = 0; i < triCount_; i++)
    idTris[size_t(i)] = uint32_t(i);
  for (int i = 0; i < vertCount_; i++)
    idVerts[size_t(i)] = uint32_t(i);
  if (!prepareNormals(co, no, idTris.data(), triCount_, idVerts.data(), vertCount_)) {
    return false;
  }
  return ctx_->runOneShot([&](VkCommandBuffer cb) { recordNormals(cb); });
}

bool GpuNormalPass::scatter(VkBuffer co,
                            VkBuffer no,
                            VkBuffer slotVertex,
                            VkBuffer pos,
                            VkBuffer nor,
...
void GpuNormalPass::recordScatter(VkCommandBuffer cb,
                                  VkBuffer co,
                                  VkBuffer no,
                                  VkBuffer slotVertex,
                                  VkBuffer pos,
                                  ...
                                  int slotCount)
{
  ...
  vkCmdBindDescriptorSets(
      cb, VK_PIPELINE_BIND_POINT_COMPUTE, scatter_.layout, 0, 1, &set, 0, nullptr);
  vkCmdPushConstants(
      cb, scatter_.layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &count);
  vkCmdDispatch(cb, groups, 1, 1);
}

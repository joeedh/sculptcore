...
static void planePre(CommandCtxBase &ctx, std::span<typename TYPES::node_type *> nodes)
{
...
template <CommandTypes TYPES>
static void planePost(CommandCtxBase &ctx, std::span<typename TYPES::node_type *> nodes)
{
  (void)ctx;
  (void)nodes;
}
...
static void createPlaneBrush(BrushCommandDef<CommandCtx<TYPES>> &def)
{
  ...
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{
      "planeSide", true, false, 0.0f, false, 0.0f, 0.0f});
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{
...
inline void packPlaneGpuUniforms(sculptcore::brush::Brush &brush, unsigned char *out)
{
  (void)brush;
  (void)out;
  std::memcpy(out + 72, &brush.planeoff, 4);
  std::memcpy(out + 76, &brush.planeSide, 4);
}

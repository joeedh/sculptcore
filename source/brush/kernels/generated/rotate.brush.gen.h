...
static void rotatePre(CommandCtxBase &ctx, std::span<typename TYPES::node_type *> nodes)
{
...
static void rotate(CommandCtx<TYPES> &ctx)
{
  ...
  for (auto &v : ctx.template vertexIter<AccMode>(ctx.node)) {
    ...
    float a = (ctx.brush.rotateAngle * fall);
    float3 k = (ctx.surfaceNo).normalized();
    float3 d = (v.co - ctx.surfacePos);
    float c = std::cos(a);
    float sn = std::sin(a);
    float3 r = (((d * c) + ((k).cross(d) * sn)) + (k * (((k).dot(d) * ((1.0f - c))))));
    v.co += (r - d);
...
template <CommandTypes TYPES>
static void rotatePost(CommandCtxBase &ctx, std::span<typename TYPES::node_type *> nodes)
{
  (void)ctx; (void)nodes;
}

template <CommandTypes TYPES, sculptcore::brush::AccumMode AccMode = sculptcore::brush::AccumLive>
static void createRotateBrush(BrushCommandDef<CommandCtx<TYPES>> &def)
{
  def.execPre  = rotatePre<TYPES>;
  def.exec     = rotate<TYPES, AccMode>;
  def.execPost = rotatePost<TYPES>;
  ...
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{"rotateAngle", true, false, 0.0, false, 0.0, 0.0, -1, sculptcore::props::Prop::FLOAT32, false});
...
inline void packRotateGpuUniforms(sculptcore::brush::Brush &brush, unsigned char *out)
{
  (void)brush;
  (void)out;
  std::memcpy(out + 72, &brush.rotateAngle, 4);
}

...
static void layerdrawPre(CommandCtxBase &ctx,
...
static void layerdraw(CommandCtx<TYPES> &ctx)
{
  ...
  auto *__attr_slayer = ctx.template boundAttr<float3>("slayer");
  (void)__attr_slayer;
  for (auto &v : ctx.template vertexIter<AccMode>(ctx.node)) {
    ...
    (*__attr_slayer)[v.v] =
        ((*__attr_slayer)[v.v] + (ctx.surfaceNo * (((s * ctx.brush.radius) * 0.5f))));
...
template <CommandTypes TYPES>
static void layerdrawPost(CommandCtxBase &ctx,
                          std::span<typename TYPES::node_type *> nodes)
{
  (void)ctx;
  (void)nodes;
...
static void createLayerdrawBrush(BrushCommandDef<CommandCtx<TYPES>> &def)
{
  ...
  def.attrs.append(sculptcore::brush::BrushAttrManifestEntry{
      ...
      true,
      true,
      int(sculptcore::mesh::AttrUse::SCULPT_LAYER)});
...
inline void packLayerdrawGpuUniforms(sculptcore::brush::Brush &brush, unsigned char *out)
{
  (void)brush;
  (void)out;
}

...
static void wingscrapePre(CommandCtxBase &ctx,
...
static void wingscrape(CommandCtx<TYPES> &ctx)
{
  ...
  for (auto &v : ctx.template vertexIter<AccMode>(ctx.node)) {
    ...
    float3 P =
        (ctx.surfacePos + (ctx.surfaceNo * ((ctx.brush.planeoff * ctx.brush.radius))));
...
template <CommandTypes TYPES>
static void wingscrapePost(CommandCtxBase &ctx,
                           std::span<typename TYPES::node_type *> nodes)
{
  (void)ctx;
  (void)nodes;
...
static void createWingscrapeBrush(BrushCommandDef<CommandCtx<TYPES>> &def)
{
  ...
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{
      "wingAngle", true, false, 0.0f, false, 0.0f, 0.0f});
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{
      "planeoff", true, true, 0.0f, false, 0.0f, 0.0f});
  def.uniforms.append(sculptcore::brush::BrushUniformManifestEntry{
      "radius", true, true, 0.0f, false, 0.0f, 0.0f});
  def.registerProps = [](sculptcore::props::StructDef &sd) {
    if (!sd.has("planeoff"))
      sd.Float32("planeoff", "planeoff").Default(0.0f);
    if (!sd.has("radius"))

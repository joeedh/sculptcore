...
// the same helper/math surface for either target; OpenCL stays nullptr.
...
static IntrinsicDef sIntrinsicsRaw[] = {
  ...
  INTR_CWG("strength", TypeKind::Float,  1, ARG1(TypeKind::Float3),
          "ctx.strength($0)",         "brush_strength($0)", "brush_strength($0)"),
  ...
  INTR_CWG("falloff",  TypeKind::Float,  1, ARG1(TypeKind::Float),
          "ctx.brush.falloffEval($0)", "brush_falloff($0)", "brush_falloff($0)"),
  ...
  INTR_CWG("sampleBrushTex", TypeKind::Float, 2, ARG2(TypeKind::Float3, TypeKind::Float3),
          "ctx.sampleBrushTex($0, $1)", "brush_sample_tex($0, $1)", "brush_sample_tex($0, $1)"),
  ...
  INTR_CWG("length",    TypeKind::Float,  1, ARG1(TypeKind::Float3),
          "($0).length()",            "length($0)",        "sc_length($0)"),
  INTR_CWG("dot",       TypeKind::Float,  2, ARG2(TypeKind::Float3, TypeKind::Float3),
          "($0).dot($1)",             "dot($0, $1)",       "sc_dot($0, $1)"),
  INTR_CWG("normalize", TypeKind::Float3, 1, ARG1(TypeKind::Float3),
          "($0).normalized()",        "normalize($0)",     "sc_normalize($0)"),
  INTR_CWG("cross",     TypeKind::Float3, 2, ARG2(TypeKind::Float3, TypeKind::Float3),
          "($0).cross($1)",           "cross($0, $1)",     "sc_cross($0, $1)"),
  INTR_CWG("distance",  TypeKind::Float,  2, ARG2(TypeKind::Float3, TypeKind::Float3),
          "(($1) - ($0)).length()",   "distance($0, $1)",  "sc_length(($1) - ($0))"),
  INTR_CWG("mix",       TypeKind::Float,  3, ARG3(TypeKind::Float, TypeKind::Float, TypeKind::Float),
          "(($0) + (($1) - ($0)) * ($2))", "mix($0, $1, $2)", "(($0) + (($1) - ($0)) * ($2))"),

  INTR_CWG("min",       TypeKind::Float,  2, ARG2(TypeKind::Float, TypeKind::Float),
          "std::min($0, $1)",         "min($0, $1)",       "fminf($0, $1)"),
  INTR_CWG("max",       TypeKind::Float,  2, ARG2(TypeKind::Float, TypeKind::Float),
          "std::max($0, $1)",         "max($0, $1)",       "fmaxf($0, $1)"),
  INTR_CWG("clamp",     TypeKind::Float,  3, ARG3(TypeKind::Float, TypeKind::Float, TypeKind::Float),
          "std::clamp<float>($0, $1, $2)", "clamp($0, $1, $2)", "sb_clampf($0, $1, $2)"),
  INTR_CWG("abs",       TypeKind::Float,  1, ARG1(TypeKind::Float),
          "std::abs($0)",             "abs($0)",           "fabsf($0)"),
  INTR_CWG("sqrt",      TypeKind::Float,  1, ARG1(TypeKind::Float),
          "std::sqrt($0)",            "sqrt($0)",          "sqrtf($0)"),
  ...
  // x - floor(x).
  INTR_CWG("sin",       TypeKind::Float,  1, ARG1(TypeKind::Float),
          "std::sin($0)",             "sin($0)",           "sinf($0)"),
  INTR_CWG("cos",       TypeKind::Float,  1, ARG1(TypeKind::Float),
          "std::cos($0)",             "cos($0)",           "cosf($0)"),
  INTR_CWG("floor",     TypeKind::Float,  1, ARG1(TypeKind::Float),
          "std::floor($0)",           "floor($0)",         "floorf($0)"),
  INTR_CWG("fract",     TypeKind::Float,  1, ARG1(TypeKind::Float),
          "(($0) - std::floor($0))",  "fract($0)",         "(($0) - floorf($0))"),
};

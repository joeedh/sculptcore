...
const CIntrinsic *findCIntrinsic(const char *name)
{
  for (const auto &ci : kCIntrinsics) {
    if (std::strcmp(ci.name, name) == 0) return &ci;
  }
  return nullptr;
}
...
struct Emit {
  ...
  {
    for (const auto &l : locals) {
      if (string(l.name).operator==(string(name.c_str()))) return l.type;
    }
  ...
  {
    if (!currentStage) return TypeKind::Unknown;
    for (const auto &p : currentStage->params) {
      if (string(p.name).operator==(string(name.c_str()))) return p.type;
    }
    return TypeKind::Unknown;
  }
  ...
  bool anyTextureUsesRamp() const
  {
    for (const auto &t : brush->textures) {
      for (const auto &tp : t.texParams) {
        if (tp.kind == TexParamKind::Ramp) return true;
      }
    }
    return false;
  }
  ...
  {
    switch (e.kind) {
    case ExprKind::LitFloat: return TypeKind::Float;
    case ExprKind::LitInt: return TypeKind::Int;
    case ExprKind::LitBool: return TypeKind::Bool;
    case ExprKind::Paren: return e.lhs ? resolveExprType(*e.lhs) : TypeKind::Unknown;
    case ExprKind::Ident: {
      stringref nm(e.name.c_str());
      for (const auto &l : locals) {
        if (string(l.name).operator==(string(nm.c_str()))) return l.type;
      }
      if (isStageParam(nm)) return stageParamType(nm);
    ...
    case ExprKind::Member: {
      ...
      if (isVectorType(bt) && swizzleIndex(e.name.c_str()) >= 0) return TypeKind::Float;
      return TypeKind::Unknown;
    }
    case ExprKind::Index:
      return (e.lhs && isVectorType(resolveExprType(*e.lhs))) ? TypeKind::Float : TypeKind::Unknown;
    case ExprKind::Unary: return e.lhs ? resolveExprType(*e.lhs) : TypeKind::Unknown;
    case ExprKind::Binary: {
      TypeKind a = e.lhs ? resolveExprType(*e.lhs) : TypeKind::Unknown;
      TypeKind b = e.rhs ? resolveExprType(*e.rhs) : TypeKind::Unknown;
      if (isVectorType(a)) return a;
      if (isVectorType(b)) return b;
      return a != TypeKind::Unknown ? a : b;
    }
    case ExprKind::Call: {
      const char *n = e.name.c_str();
      if (std::strcmp(n, "float3") == 0) return TypeKind::Float3;
      if (const TextureDef *td = findTextureCall(stringref(n))) return td->returnType;
      if (std::strcmp(n, "mapPoint") == 0) return TypeKind::Float3;
  ...
  void emitExpr(const Expr &e)
  {
    ...
    case ExprKind::LitBool: out += e.bvalue ? "1" : "0"; break;
    case ExprKind::Ident: {
      stringref nm(e.name.c_str());
      ...
      if (isLocal(nm) || isStageParam(nm)) { out += e.name; break; }
      if (const TexParam *tp = findTexParam(nm)) {
        if (tp->isConst) {
    ...
    case ExprKind::Member: {
      if (e.lhs && isVectorType(resolveExprType(*e.lhs)) && swizzleIndex(e.name.c_str()) == 3) {
    ...
    case ExprKind::Index:
      if (e.lhs && resolveExprType(*e.lhs) == TypeKind::Float3) {
        out += "sb3_idx("; emitExpr(*e.lhs); out += ", "; emitExpr(*e.rhs); out += ")";
      } else {
        emitExpr(*e.lhs); out += "["; emitExpr(*e.rhs); out += "]";
      }
      break;
    case ExprKind::Binary: {
      ...
      bool arith = e.binop == BinOp::Add || e.binop == BinOp::Sub || e.binop == BinOp::Mul ||
                   e.binop == BinOp::Div;
      if ((av || bv) && arith) {
        if (av && bv) {
          const char *f = e.binop == BinOp::Add ? "sb3_add" :
                          e.binop == BinOp::Sub ? "sb3_sub" :
                          e.binop == BinOp::Mul ? "sb3_mul" : "sb3_div";
          out += f; out += "("; emitExpr(*e.lhs); out += ", "; emitExpr(*e.rhs); out += ")";
        } else if (e.binop == BinOp::Mul) {
          out += "sb3_scale(";
          if (av) { emitExpr(*e.lhs); out += ", "; emitExpr(*e.rhs); }
          else { emitExpr(*e.rhs); out += ", "; emitExpr(*e.lhs); }
          out += ")";
        } else if (e.binop == BinOp::Div && av) {
          out += "sb3_divs("; emitExpr(*e.lhs); out += ", "; emitExpr(*e.rhs); out += ")";
        } else {
    ...
    case ExprKind::Unary:
      if (e.unaryop == UnaryOp::Neg && e.lhs && resolveExprType(*e.lhs) == TypeKind::Float3) {
        err("unary minus on float3 is not supported (use v * -1.0)");
        out += "sb_f3(0.0f, 0.0f, 0.0f)";
        break;
      }
      out += "("; out += unaryOpCSym(e.unaryop); emitExpr(*e.lhs); out += ")";
      break;
    case ExprKind::Paren: out += "("; emitExpr(*e.lhs); out += ")"; break;
    case ExprKind::Call: {
      const char *n = e.name.c_str();
      if (std::strcmp(n, "float3") == 0) {
        if (e.args.size() != 3) { err("float3() takes exactly three arguments"); out += "sb_f3(0.0f, 0.0f, 0.0f)"; break; }
        out += "sb_f3(";
        for (int i = 0; i < 3; i++) { if (i) out += ", "; emitExpr(*e.args[i]); }
        out += ")";
        break;
      }
      ...
      if (findTextureCall(stringref(e.name.c_str()))) {
        if (currentTexture) errf("texture '%s' cannot call another texture", currentTexture->name.c_str());
        else err("texture calls are only valid inside a texture eval on the C backend");
        out += "0.0f";
        break;
      }
      if (std::strcmp(n, "mapPoint") == 0) {
        if (!currentTexture) err("mapPoint() is only valid inside a texture eval");
        if (e.args.size() != 1) {
          err("mapPoint() takes exactly one float3 argument");
          out += "sb_f3(0.0f, 0.0f, 0.0f)";
          break;
        }
        out += "texMapPoint(sb_texctx, "; emitExpr(*e.args[0]); out += ")";
        break;
      }
      if (currentTexture) {
        const char *dot = std::strchr(n, '.');
        if (dot && std::strcmp(dot, ".sample") == 0) {
          ...
          if (tp && tp->kind == TexParamKind::Ramp && e.args.size() == 1) {
            char buf[64];
            std::snprintf(buf, sizeof(buf), "texRampSample(sb_tex_params + %d, ", tp->offset);
            out += buf; emitExpr(*e.args[0]); out += ")";
            break;
          }
        }
      ...
      if (const CIntrinsic *ci = findCIntrinsic(n)) {
        ...
        for (const auto &a : e.args) { rendered.append(render(*a)); }
      ...
      if (findIntrinsic(stringref(n))) {
        errf("intrinsic '%s' requires brush context and is not available in a texture script", n);
        out += "0.0f";
        break;
      }
      out += e.name; out += "(";
  ...
  void emitDual(const Expr &e)
  {
    ...
    case ExprKind::Member:
      if (e.lhs && isGradVar(*e.lhs)) {
        out += "sb_comp(sb_seed3("; emitExpr(*e.lhs); out += "), ";
    ...
    case ExprKind::Call: {
      ...
      if (std::strcmp(n, "mapPoint") == 0 && e.args.size() == 1) {
        if (!currentTexture) err("mapPoint() is only valid inside a texture eval");
        out += "sbd_mapPoint(sb_texctx, "; emitDual(*e.args[0]); out += ")";
        break;
      }
  ...
  void emitStmt(const Stmt &s)
  {
    ...
    case StmtKind::DeclLocal:
      ...
      if (!cTypeName(s.declType) && s.declType != TypeKind::Unknown) {
        errf("local '%s' has a type unsupported on the C texture backend", s.name.c_str());
        break;
      }
      writeIndent();
      ...
      if (resolveExprType(*s.lvalue) == TypeKind::Float3) {
        ...
        if (opc != '=') {
          ...
          const char *f = opc == '+' ? "sb3_add" :
                          opc == '-' ? "sb3_sub" :
                          opc == '*' ? (rt == TypeKind::Float3 ? "sb3_mul" : "sb3_scale") :
                                       (rt == TypeKind::Float3 ? "sb3_div" : "sb3_divs");
          out += f; out += "("; out += lv; out += ", "; emitExpr(*s.rvalue); out += ")";
        } else emitExpr(*s.rvalue);
        out += ";\n";
        break;
      }
    ...
    case StmtKind::Continue: writeIndent(); out += "continue;\n"; break;
  ...
  void emitPrelude()
  {
    ...
    write("static float3 sb_f3(float x, float y, float z) { float3 r; r.x = x; r.y = y; r.z = z; return r; }\n");
    write("static float sb3_idx(float3 a, int i) { return i == 0 ? a.x : i == 1 ? a.y : a.z; }\n");
    write("static float3 sb3_add(float3 a, float3 b) { return sb_f3(a.x + b.x, a.y + b.y, a.z + b.z); }\n");
    write("static float3 sb3_sub(float3 a, float3 b) { return sb_f3(a.x - b.x, a.y - b.y, a.z - b.z); }\n");
    write("static float3 sb3_mul(float3 a, float3 b) { return sb_f3(a.x * b.x, a.y * b.y, a.z * b.z); }\n");
    write("static float3 sb3_div(float3 a, float3 b) { return sb_f3(a.x / b.x, a.y / b.y, a.z / b.z); }\n");
    write("static float3 sb3_scale(float3 a, float s) { return sb_f3(a.x * s, a.y * s, a.z * s); }\n");
    write("static float3 sb3_divs(float3 a, float s) { return sb_f3(a.x / s, a.y / s, a.z / s); }\n");
    write("static float3 sb3_mix(float3 a, float3 b, float t) { return sb3_add(a, sb3_scale(sb3_sub(b, a), t)); }\n");
    ...
    write("  return sb_f3((float)((double)a.x * mul), (float)((double)a.y * mul), (float)((double)a.z * mul));\n");
    write("}\n");
    write("static float3 sc_cross(float3 a, float3 b) {\n");
    write("  return sb_f3(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);\n");
    write("}\n\n");
    ...
    write("static sbdual sb_dual(float v, float3 d) { sbdual r; r.v = v; r.d = d; return r; }\n");
    write("static sbdual3 sb_dual3(float3 v, float3 dx, float3 dy, float3 dz) { sbdual3 r; r.v = v; r.dx = dx; r.dy = dy; r.dz = dz; return r; }\n");
    write("static sbdual sb_c(float x) { return sb_dual(x, sb_f3(0.0f, 0.0f, 0.0f)); }\n");
    write("static sbdual3 sb_c3(float3 p) { float3 z = sb_f3(0.0f, 0.0f, 0.0f); return sb_dual3(p, z, z, z); }\n");
    write("static sbdual3 sb_seed3(float3 p) { return sb_dual3(p, sb_f3(1.0f, 0.0f, 0.0f), sb_f3(0.0f, 1.0f, 0.0f), sb_f3(0.0f, 0.0f, 1.0f)); }\n");
    write("static sbdual sb_comp(sbdual3 a, int i) { return sb_dual(sb3_idx(a.v, i), sb_f3(sb3_idx(a.dx, i), sb3_idx(a.dy, i), sb3_idx(a.dz, i))); }\n");
    write("static sbdual3 sb_v3(sbdual x, sbdual y, sbdual z) { return sb_dual3(sb_f3(x.v, y.v, z.v), sb_f3(x.d.x, y.d.x, z.d.x), sb_f3(x.d.y, y.d.y, z.d.y), sb_f3(x.d.z, y.d.z, z.d.z)); }\n");
    write("static sbdual sbd_add(sbdual a, sbdual b) { return sb_dual(a.v + b.v, sb3_add(a.d, b.d)); }\n");
    write("static sbdual sbd_sub(sbdual a, sbdual b) { return sb_dual(a.v - b.v, sb3_sub(a.d, b.d)); }\n");
    write("static sbdual sbd_neg(sbdual a) { return sb_dual(-a.v, sb3_scale(a.d, -1.0f)); }\n");
    write("static sbdual sbd_mul(sbdual a, sbdual b) { return sb_dual(a.v * b.v, sb3_add(sb3_scale(a.d, b.v), sb3_scale(b.d, a.v))); }\n");
    write("static sbdual sbd_div(sbdual a, sbdual b) { return sb_dual(a.v / b.v, sb3_divs(sb3_sub(sb3_scale(a.d, b.v), sb3_scale(b.d, a.v)), b.v * b.v)); }\n");
    write("static sbdual sbd_sin(sbdual a) { return sb_dual(sinf(a.v), sb3_scale(a.d, cosf(a.v))); }\n");
    write("static sbdual sbd_cos(sbdual a) { return sb_dual(cosf(a.v), sb3_scale(a.d, -sinf(a.v))); }\n");
    write("static sbdual sbd_sqrt(sbdual a) { float r = sqrtf(a.v); return sb_dual(r, sb3_scale(a.d, r > 0.0f ? 0.5f / r : 0.0f)); }\n");
    write("static sbdual sbd_abs(sbdual a) { return sb_dual(fabsf(a.v), sb3_scale(a.d, a.v < 0.0f ? -1.0f : 1.0f)); }\n");
    write("static sbdual sbd_dot(sbdual3 a, sbdual3 b) {\n");
    write("  float3 d = sb3_add(sb3_add(sb3_add(sb3_add(sb3_add(sb3_scale(a.dx, b.v.x), sb3_scale(b.dx, a.v.x)),\n");
    write("      sb3_scale(a.dy, b.v.y)), sb3_scale(b.dy, a.v.y)), sb3_scale(a.dz, b.v.z)), sb3_scale(b.dz, a.v.z));\n");
    ...
    if (anyTextureUsesMap()) {
      ...
      write("  return sb_f3((qx * d - q.x * qw) / (d * d), (qy * d - q.y * qw) / (d * d), (qz * d - q.z * qw) / (d * d));\n");
      write("}\n");
  ...
  {
    if (td.slabSize <= 0) return;
    char buf[64];
    std::snprintf(buf, sizeof(buf), "_param_defaults[%d] = {\n", td.slabSize);
    write("const float tex_"); write(lower(td.name)); write(buf);
  ...
  bool checkEvalSignature(const TextureDef &td)
  {
    if (td.returnType != TypeKind::Float) {
      errf("texture '%s' must return float for the C backend (runtime eval ABI)", td.name.c_str());
      return false;
    }
    for (const auto &p : td.params) {
      if (p.type != TypeKind::Float && p.type != TypeKind::Float3) {
        errf("texture eval param '%s' must be float or float3 on the C backend", p.name.c_str());
        return false;
      }
    }
    return true;
  ...
  {
    if (!checkEvalSignature(td)) return;
    string name = texEvalName(td);

    write("static float "); write(name); write("_impl(");
    for (const auto &p : td.params) {
      write(p.type == TypeKind::Float3 ? "float3 " : "float ");
      write(p.name); write(", ");
    }
    write("const float *sb_tex_params, const TexEvalCtx *sb_texctx)\n{\n");
    for (const auto &p : td.params) { write("  (void)"); write(p.name); write(";\n"); }
    write("  (void)sb_tex_params;\n  (void)sb_texctx;\n");
    emitBody(td);
    write("}\n\n");

    write("float "); write(name); write("(");
    for (const auto &p : td.params) {
      write(p.type == TypeKind::Float3 ? "const float *" : "float ");
      write(p.name); write(", ");
    }
    write("const float *sb_tex_params, const TexEvalCtx *sb_texctx)\n{\n");
    write("  return "); write(name); write("_impl(");
    for (const auto &p : td.params) {
      if (p.type == TypeKind::Float3) {
        write("sb_f3("); write(p.name); write("[0], "); write(p.name); write("[1], ");
        write(p.name); write("[2])");
      } else {
        write(p.name);
      }
      write(", ");
  ...
  {
    if (td.returnType != TypeKind::Float) return;
    int outMark = (int)out.size();
    int errMark = (int)errors.size();
    string name = texEvalName(td);

    write("static sbdual "); write(name); write("_d_impl(");
    for (const auto &p : td.params) {
    ...
    write("  (void)sb_tex_params;\n  (void)sb_texctx;\n");
    int sl = (int)locals.size();
    for (const auto &p : td.params) locals.append(LocalVar{p.name, p.type, /*dual=*/true});
    dualBody = true;
    emitBody(td);
    dualBody = false;
    while ((int)locals.size() > sl) locals.pop_back();
    ...
    write("void "); write(name); write("_d(");
    for (const auto &p : td.params) {
      write(p.type == TypeKind::Float3 ? "const sbdual3 *" : "const sbdual *");
      write(p.name); write(", ");
    }
    write("const float *sb_tex_params, const TexEvalCtx *sb_texctx, sbdual *sb_out)\n{\n");
    write("  *sb_out = "); write(name); write("_d_impl(");
    ...
    if ((int)errors.size() > errMark) {
      ...
      write("// "); write(name); write("_d omitted: eval is not differentiable\n\n");
    }
  }
};

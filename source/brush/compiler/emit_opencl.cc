...
struct Emit {
  ...
  {
    if (!vertexStage || vertexStage->params.size() == 0)
      return false;
  ...
  void emitStmt(const Stmt &s)
  {
    ...
    case StmtKind::DeclLocal:
      ...
      writeIndent();
      out += (s.declType == TypeKind::Struct) ? s.declStructName.c_str()
                                              : clType(s.declType);
      out += " ";
      out += s.name;
      if (s.expr) {
    ...
    case StmtKind::NeighborLoop: {
      ...
      string metaVar = string("sb_nbr_meta") + sfx, niVar = string("sb_ni") + sfx,
             idxVar = string("sb_nb_v") + sfx;
      writeIndent();
      out += "{\n";
      indent++;
      writeIndent();
      out += "uint2 " + metaVar + " = vert_nbr_meta[" + outerIdx + "];\n";
      writeIndent();
      out += "for (uint " + niVar + " = 0u; " + niVar + " < " + metaVar + ".y; " + niVar +
             " = " + niVar + " + 1u) {\n";
      indent++;
      writeIndent();
      out += "uint " + idxVar + " = nbr_verts[" + metaVar + ".x + " + niVar + "];\n";
  ...
  void emitPrelude()
  {
    ...
    if (brushUsesGrad()) {
      ...
      write(
          "inline float sb_idx(float3 a, int i) { return (i==0)?a.x:(i==1)?a.y:a.z; }\n");
      write("inline sbdual sb_c(float x) { sbdual r; r.v=x; r.d=(float3)(0.0f); return "
            "r; }\n");
      write("inline sbdual3 sb_c3(float3 p) { sbdual3 r; r.v=p; r.dx=(float3)(0.0f); "
            "r.dy=(float3)(0.0f); r.dz=(float3)(0.0f); return r; }\n");
      write("inline sbdual3 sb_seed3(float3 p) { sbdual3 r; r.v=p; "
            "r.dx=(float3)(1.0f,0.0f,0.0f); r.dy=(float3)(0.0f,1.0f,0.0f); "
            "r.dz=(float3)(0.0f,0.0f,1.0f); return r; }\n");
      write(
          "inline sbdual sb_comp(sbdual3 a, int i) { sbdual r; r.v=sb_idx(a.v,i); "
          "r.d=(float3)(sb_idx(a.dx,i), sb_idx(a.dy,i), sb_idx(a.dz,i)); return r; }\n");
      write("inline sbdual3 sb_v3(sbdual x, sbdual y, sbdual z) { sbdual3 r; "
            "r.v=(float3)(x.v,y.v,z.v); r.dx=(float3)(x.d.x,y.d.x,z.d.x); "
            "r.dy=(float3)(x.d.y,y.d.y,z.d.y); r.dz=(float3)(x.d.z,y.d.z,z.d.z); return "
            "r; }\n");
      write("inline sbdual sbd_add(sbdual a, sbdual b) { sbdual r; r.v=a.v+b.v; "
            "r.d=a.d+b.d; return r; }\n");
      write("inline sbdual sbd_sub(sbdual a, sbdual b) { sbdual r; r.v=a.v-b.v; "
            "r.d=a.d-b.d; return r; }\n");
      write("inline sbdual sbd_neg(sbdual a) { sbdual r; r.v=-a.v; r.d=-a.d; return r; "
            "}\n");
      write("inline sbdual sbd_mul(sbdual a, sbdual b) { sbdual r; r.v=a.v*b.v; "
            "r.d=a.d*b.v + b.d*a.v; return r; }\n");
      write("inline sbdual sbd_div(sbdual a, sbdual b) { sbdual r; r.v=a.v/b.v; "
            "r.d=(a.d*b.v - b.d*a.v)/(b.v*b.v); return r; }\n");
      write("inline sbdual sbd_sin(sbdual a) { sbdual r; r.v=sin(a.v); r.d=a.d*cos(a.v); "
            "return r; }\n");
      write("inline sbdual sbd_cos(sbdual a) { sbdual r; r.v=cos(a.v); "
            "r.d=a.d*(-sin(a.v)); return r; }\n");
      write("inline sbdual sbd_sqrt(sbdual a) { float rr=sqrt(a.v); sbdual r; r.v=rr; "
            "r.d=a.d*(rr>0.0f?0.5f/rr:0.0f); return r; }\n");
      write("inline sbdual sbd_abs(sbdual a) { sbdual r; r.v=fabs(a.v); "
            "r.d=a.d*(a.v<0.0f?-1.0f:1.0f); return r; }\n");
      write("inline sbdual sbd_dot(sbdual3 a, sbdual3 b) { sbdual r; r.v=dot(a.v,b.v); "
            "r.d=a.dx*b.v.x+b.dx*a.v.x+a.dy*b.v.y+b.dy*a.v.y+a.dz*b.v.z+b.dz*a.v.z; "
            "return r; }\n");
      write("inline sbdual sbd_length(sbdual3 a) { return sbd_sqrt(sbd_dot(a,a)); }\n");
      write("inline sbdual sbd_mix(sbdual a, sbdual b, sbdual t) { return sbd_add(a, "
            "sbd_mul(sbd_sub(b,a), t)); }\n");
      write("inline sbdual sbd_floor(sbdual a) { sbdual r; r.v=floor(a.v); "
            "r.d=(float3)(0.0f); return r; }\n");
      write("inline sbdual sbd_fract(sbdual a) { sbdual r; r.v=a.v-floor(a.v); r.d=a.d; "
            "return r; }\n\n");
    }
    ...
    for (const auto &f : brush->fields) {
      if (f.kind != FieldKind::Uniform || isBuiltinBrushName(f.name.c_str()))
        continue;
      writeFieldMember(f);
    }
    write("} BrushUniforms;\n\n");
    ...
    for (const auto &f : brush->fields) {
      if (f.kind != FieldKind::Ctx || isBuiltinCtxName(f.name.c_str()))
        continue;
      writeFieldMember(f);
    }
    write("} CtxUniforms;\n\n");
    ...
    write("inline float sb_falloff(__constant BrushUniforms* bu, __constant float* lut, "
          "float t) {\n");
    write("  if (bu->falloff_kind == 1u) return t;\n");
    write("  if (bu->falloff_kind == 2u) { float u = 1.0f - t; return exp(-9.0f * u * "
          "u); }\n");
    write("  if (bu->falloff_kind == 3u) { float c = clamp(t,0.0f,1.0f); float "
          "s=c*255.0f; int i=(int)floor(s);\n");
    write("    if (i >= 255) return lut[255]; float f = s - (float)i; return "
          "lut[i]*(1.0f-f) + lut[i+1]*f; }\n");
    ...
    write("  if (bu->falloff_shape == 1u) { float3 a = fabs(d); return fmax(a.x, "
          "fmax(a.y, a.z)) * ir; }\n");
    write("  if (bu->falloff_shape == 2u) return fabs(dot(d, bu->falloff_dir)) * ir;\n");
    write("  return length(d) * ir;\n");
    write("}\n");
    write("inline float2 sb_stroke_uv(__constant BrushUniforms* bu, __global "
          "StrokeSample* sp, float3 co) {\n");
    write("  if (bu->stroke_path_count == 0u) return (float2)(0.0f, 0.0f);\n");
    write("  if (bu->stroke_path_count == 1u) return (float2)(sp[0].arclen, length(co - "
          "sp[0].pos));\n");
    write("  float bd = 3.402823e+38f, ba = 0.0f, bl = 0.0f;\n");
    write("  for (uint i = 0u; i + 1u < bu->stroke_path_count; i = i + 1u) {\n");
    write("    float3 a = sp[i].pos; float3 ab = sp[i+1u].pos - a; float l2 = dot(ab, "
          "ab); float t = 0.0f;\n");
    write("    if (l2 > 0.0f) t = dot(co - a, ab) / l2; t = clamp(t, 0.0f, 1.0f);\n");
    write("    float dd = length(co - (a + ab*t)); if (dd < bd) { bd = dd; ba = "
          "sp[i].arclen + (sp[i+1u].arclen - sp[i].arclen)*t; bl = dd; }\n");
    write("  }\n");
    write("  return (float2)(ba, bl);\n");
    write("}\n");
    write("inline float sb_sample_tex(__constant BrushUniforms* bu, __constant "
          "CtxUniforms* cu, __global StrokeSample* sp,\n");
    write("                           __global float* tex, int w, int h, float3 co, "
          "float3 no) {\n");
    write("  (void)no; float2 uv;\n");
    write("  if (bu->coord_space == 1u) { float3 p = "
          "sb_mat4_mul_point(cu->render_matrix, co); uv = (float2)(p.x, p.y); }\n");
    // Aspect-corrected tiled repeat; mirrors CommandCtx::sampleBrushTex.
    write("  else if (bu->coord_space == 2u) { float3 p = "
          "sb_mat4_mul_point(cu->render_matrix, co);\n");
    ...
    write("  else if (bu->coord_space == 4u) { float3 n = normalize(cu->surfaceNo); "
          "float3 r = (float3)(0.0f,0.0f,1.0f);\n");
    write("    if (fabs(n.z) >= 0.999f) r = (float3)(1.0f,0.0f,0.0f); float3 t1 = "
          "normalize(cross(r,n)); float3 t2 = cross(n,t1);\n");
    write("    float3 rel = co - cu->surfacePos; uv = (float2)(dot(rel,t1), "
          "dot(rel,t2)); }\n");
    ...
    write("  float fx = uv.x*(float)w - 0.5f, fy = uv.y*(float)h - 0.5f; float x0 = "
          "floor(fx), y0 = floor(fy);\n");
    write("  float tx = fx-x0, ty = fy-y0;\n");
    write("  int x0c=(int)clamp(x0,0.0f,(float)(w-1)), "
          "y0c=(int)clamp(y0,0.0f,(float)(h-1));\n");
    write("  int x1c=(int)clamp(x0+1.0f,0.0f,(float)(w-1)), "
          "y1c=(int)clamp(y0+1.0f,0.0f,(float)(h-1));\n");
    write("  float p00=tex[y0c*w+x0c], p10=tex[y0c*w+x1c], p01=tex[y1c*w+x0c], "
          "p11=tex[y1c*w+x1c];\n");
    ...
    write("inline float sb_strength(__constant BrushUniforms* bu, __constant "
    ...
    write("#define brush_strength(p) sb_strength(brush_u, ctx_u, falloff_lut, "
          "stroke_path, brush_tex, brush_tex_w, brush_tex_h, (p))\n");
    ...
    write("#define brush_sample_tex(c, n) sb_sample_tex(brush_u, ctx_u, stroke_path, "
          "brush_tex, brush_tex_w, brush_tex_h, (c), (n))\n\n");
  }
  ...
  void emitTextureFn(const TextureDef &td)
  {
    if (td.texParams.size() > 0 || td.usesMap || td.samplerDeps.size() > 0) {
      errf(
          "texture '%s' uses params/mapPoint/samplers, unsupported on the opencl backend",
          td.name.c_str());
      return;
    }
    write("inline ");
    write(clType(td.returnType));
    write(" ");
  ...
  void emitReduceStage(const Stage &st)
  {
    ...
    for (const auto &p : st.params) {
      ...
      first = false;
      const char *ts =
          (p.type == TypeKind::Struct) ? p.structName.c_str() : clType(p.type);
      write(ts);
      if (p.dir == ParamDir::Out || p.dir == ParamDir::InOut) {
        write("* ");
        ptrParams.append(p.name);
      } else
        write(" ");
      write(p.name);
    }
    write(", __constant BrushUniforms* brush_u, __constant CtxUniforms* ctx_u, "
          "__constant float* falloff_lut,");
    write(" __global StrokeSample* stroke_path, __global float* brush_tex, int "
          "brush_tex_w, int brush_tex_h) {\n");
    indent = 1;
    currentStage = &st;
  ...
  void run()
  {
    ...
    write(kernelName);
    write("(\n");
    write("    __global float3* co_buf, __global float3* no_buf, __global float* "
          "mask_buf,\n");
    write("    __global uint* unique_verts, __global NodeMeta* nodes,\n");
    write("    __constant BrushUniforms* brush_u, __constant CtxUniforms* ctx_u, "
          "__constant float* falloff_lut,\n");
    write("    __global float* brush_tex, int brush_tex_w, int brush_tex_h, __global "
          "StrokeSample* stroke_path");
    if (usesNeighbors)
      write(",\n    __global float3* co_prev, __global uint2* vert_nbr_meta, __global "
            "uint* nbr_verts");
    write(") {\n");
    ...
    for (int pi = 1; pi < (int)vertexStage->params.size(); pi++) {
      ...
      write("  ");
      write(p.type == TypeKind::Struct ? p.structName.c_str() : clType(p.type));
      write(" ");
      write(p.name);
      write(";\n");
    }
    for (const auto *st : reduceStages) {
      ...
      write(", brush_u, ctx_u, falloff_lut, stroke_path, brush_tex, brush_tex_w, "
            "brush_tex_h);\n");
    }
    write("\n");

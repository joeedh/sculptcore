              ...
              std::string &err)
{
  ...
  if (verb == "remesh") {
    /* remesh [target=..] [curvature=1] [sharp=1] [sharp_angle=..] [density=0]
  ...
  if (verb == "remesh_cross_field") {
     ...
     * [curvature_weight=..] — solve the 4-RoSy cross field; prints face count,
     * singularity count, Σ index (== 4χ) and whether the eigen fallback ran. */
    if (!scene.mesh) {
  ...
  if (verb == "remesh_quantize") {
     ...
     *   [integer_tol=..] [max_lambda=..] [max_iters=..] — integer-grid
     * quantization (M5); builds the field/cut/seamless system internally. Prints
     * the integer residual, one-ring loop-closure residual and feasibility. */
    if (!scene.mesh) {
    ...
    qp.max_iters = getInt(args, "max_iters", qp.max_iters);

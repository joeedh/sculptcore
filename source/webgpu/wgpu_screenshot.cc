                        ...
                        int *outNonUniformPixels)
{
  if (outNonUniformPixels)
    *outNonUniformPixels = 0;
  if (!ctx.device || target.width <= 0 || target.height <= 0 || !target.colorTexture) {
    return false;
  }
  ...
  if (mreq.ok) {
    ...
    if (mapped) {
      ...
      if (pixels) {
        ...
        if (!ok)
          fprintf(stderr, "captureTargetToPNG: stbi_write_png failed for '%s'\n", path);
        std::free(pixels);
      }
    }

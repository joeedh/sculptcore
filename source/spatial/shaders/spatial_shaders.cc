...
litestl::binding::types::Struct<SpatialShaders> *SpatialShaders::defineBindings()
{
  ...
  BIND_STRUCT_MEMBER(st, basicPointShader);
...
static gpu::UniformBlockDef *makeDefaultBlock(bool withNormalMatrix)
{
  ...
  auto *block = New<gpu::UniformBlockDef>(
...
/* The point-sprite UBO adds `viewportSize` (xy, in a vec4 for safe std140
 * alignment) so the vertex stage can size billboards in constant pixels. */
static gpu::UniformBlockDef *makePointBlock()
{
  using namespace litestl::math;
  using litestl::alloc::New;
  ...
  fields.append(New<gpu::UniformDef<float4>>(
      "UniformDef", "uColor", gpu::GPUType::FLOAT32, 4, float4(1.0f, 1.0f, 1.0f, 1.0f)));
  fields.append(New<gpu::UniformDef<float4>>("UniformDef",
                                             "viewportSize",
                                             gpu::GPUType::FLOAT32,
                                             4,
...
gpu::ShaderDef makeBasicPointShader()
{
  using namespace litestl::math;
  litestl::alloc::PermanentGuard permGuard;
  gpu::ShaderDef def("Basic Point Shader",
                     shaders::generated::point_sprite_wgsl,
                     {
                         {"position", gpu::GPUType::FLOAT32, 3},
                         {"corner", gpu::GPUType::FLOAT32, 2},
                         {"color", gpu::GPUType::FLOAT32, 4},
                     },
                     {makePointBlock()},
                     {});
#ifndef WASM
  def.spirv = shaders::generated::point_sprite_spirv;
  def.spirvSize = sizeof(shaders::generated::point_sprite_spirv) / sizeof(uint32_t);
#endif
...
SpatialShaders spatialShaders = {
    makeBasicMeshShader(), makeBasicLineShader(), makeBasicPointShader()};
} // namespace sculptcore::spatial

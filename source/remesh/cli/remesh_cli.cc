...
void usage()
{
  std::printf(
      ...
      "(default 0)\n"
      "  --feature-min-chain <n>   drop unanchored sharp chains shorter than n "
      "edges (default 0)\n"
      ...
      "  --cap-odd <0|1>         close odd holes too, paired all-quad (default 1;\n"
      "                          0 = strict all-quad, odd rims left open)\n"
...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    else if (a == "--feature-min-chain")
      params.feature_min_chain = std::atoi(next("--feature-min-chain"));
    else if (a == "--density")

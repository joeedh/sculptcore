...
/* resolve*Selection: what a box-modeling op actually consumes. With
 * `preferOpDomain` (the sculptcore.select_flush_prefer_op_domain feature flag,
 * default on) an explicit selection in the op's own domain wins outright and
 * derivation only fills an empty domain; with it off, explicit and derived are
 * merged (set union). */
 ...
 * Appends edge indices to `out` (kept verbatim from modeling_walk.h). */

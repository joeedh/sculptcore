             ...
             bool prevent_inversion = false)
{
  ...
  if (blend > 0.0f) {
    // The survivor's position ends up at merged_co, not at the blend of the two
    // endpoints. The merge policies (attr_merge.h) must reconstruct a
    // snapshot/rest column against where the vertex actually ends up.

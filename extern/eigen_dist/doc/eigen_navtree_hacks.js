...
function getNode(o, po) {
  ...
  for (var i in po.childrenData) {
    ...
    if (!nodeData[1] || nodeData[1].indexOf('#') == -1)
      // <- we added this line
  ...
  $(window).bind('hashchange', function () {
    if (window.location.hash && window.location.hash.length > 1) {
      ...
      if ($(location).attr('hash')) {
        var clslink = stripPath($(location).attr('pathname')) + ':' + $(location).attr('hash').substring(1)
...
function createIndent(o, domNode, node, level) {
  ...
  if (checkChildrenData(node)) {
    // <- we modified this line to use checkChildrenData(node) instead of node.childrenData
...
$(document).ready(function () {
  ...
  ;(function () {
    ...
    try {
      ...
      else expandNode(global_navtree_object, global_navtree_object.node.children[0].children[1], true, true)
